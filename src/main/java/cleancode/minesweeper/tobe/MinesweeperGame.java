...
public class MinesweeperGame {
    ...
    public static final String OPENED_CELL_SIGH = "■";
    ...
    private static void actOnCell(String cellInput, String userActionCol) {
        ...
        throw new AppException("잘못된 번호를 선택");

    }

    private static boolean doseUserChooseToOpenCell(String userActionCol) {
        return userActionCol.equals("1");
    }

    private static boolean doseUserChooseToPlantFlag(String userActionCol) {
    ...
    private static void checkIfGameIsOver() {
        boolean isAllOpened = isAllCellIsOpend();
        if (isAllOpened) {
    ...
    private static void changeIfGameToLose() {
        gameStatus = -1;
    }
...
//    }
    ...
    private static void open(int row, int col) {
        ...
        if (NEARBY_LAND_MINE_COUNTS[row][col] != 0) {
            ...
            BOARD[row][col] = OPENED_CELL_SIGH;
        }
        open(row - 1, col - 1);
        open(row - 1, col);
        open(row - 1, col + 1);
